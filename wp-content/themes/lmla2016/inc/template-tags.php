 ...
 */
  ...
  * @return bool
  */
 function lmla_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'lmla_categories' ) ) ) {
 		...
 		set_transient( 'lmla_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so lmla_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so lmla_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in lmla_categorized_blog.
  */
 function lmla_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'lmla_categories' );
 }
 add_action( 'edit_category', 'lmla_category_transient_flusher' );
 add_action( 'save_post',     'lmla_category_transient_flusher' );
  ...
  */
 function lmla_comment_list( $comment, $args, $depth ) {
  ...
  */
 function lmla_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
